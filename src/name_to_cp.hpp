...
namespace uni {
    namespace details {
        ...
        constexpr node read_node(std::uint32_t offset) {
            ...
            if(long_name) {
                std::uint32_t name_offset = (std::uint32_t(index[offset++]) << 8u);
            ...
            if(has_value) {
                ...
                if(has_children) {
                    ...
                    n.children_offset |= std::uint32_t(index[offset++]);
                    assert(n.children_offset < std::size(index));
                }
            }
            else {
                std::uint8_t h = index[offset++];
                ...
                h = std::uint8_t(h & ~0xC0);
                if(has_children) {
                    n.children_offset = (std::uint32_t(h) << 16u);
                    n.children_offset |= (std::uint32_t(index[offset++]) << 8u);
                    n.children_offset |= std::uint32_t(index[offset++]);
                    assert(n.children_offset < std::size(index));
                }
        ...
        struct compare_node_result {
            node n;
            bool found;
            std::uint32_t value;
        };

        constexpr compare_node_result
        compare_node(std::uint32_t offset, std::string_view name, std::uint32_t start = 0) {
            ...
            if(n.has_children()) {
                ...
                for(;;) {
                    auto child = compare_node(o, name, start);
                    if(child.found) {
                        return {n, true, child.value};
                    }
                    o += child.n.size;
                    if(!child.n.has_sibling)
                        break;
                }
            }
            return {n, false, 0};
        ...
        constexpr const generated_name_data generated_name_data_table[] = {
            ...
            {"CJK UNIFIED IDEOGRAPH-", 0x2A700, 0x2B738},
    ...
    constexpr char32_t cp_from_name(std::string_view name) {
        ...
        for(;;) {
            auto result = details::compare_node(offset, name);
            if(!result.n.is_valid())
                break;
            if(result.found) {
                //HANGUL JUNGSEONG O-E - fix up hyphen
                if(result.value == 0x116c && name.find("O-E") != std::string_view::npos)
                    return 0x1180;
                return result.value;
            }
            if(!result.n.has_sibling)
                break;
            offset += result.n.size;
        }
        return 0xFFFFFF;
    }
}
