...
def emit_bool_table(f, name, data):
    ...
    for idx, cp in enumerate(sorted(data)):
        f.write(to_hex(cp, 6))
...
def write_categories_data(characters, categories_names, file):
    ...
    # Generate 'return false' for leaf categories absent from the data.
    # The regex-support section calls cp_category_is<category::X> for ALL categories,
    # so every leaf category needs a specialization even if the data has no such code points.
    present_leaf_cats = {cat for _, cat in sorted_by_len}
    meta_long_names = set(meta_cats.keys()) | {'unassigned'}
    for shortname, longname in categories_names:
        if shortname not in present_leaf_cats and longname not in meta_long_names:
            f.write("""template <>
        constexpr bool cp_category_is<category::{0}>(char32_t /*c*/) {{
            return false; }}
        """.format(shortname))
    ...
    for name, cats in meta_cats.items():
        has_sub_cats = any(cat in cats for _, cat in sorted_by_len)
        param = "c" if has_sub_cats else "/*c*/"
        f.write("""template <>
        constexpr bool cp_category_is<category::{}>(char32_t {}) {{
            """.format(name, param))
        for _, cat in sorted_by_len:
            if cat in cats:
...
def characters_ages(characters):
    ages = sorted(list(set([float(cp.age) for cp in characters if cp.age != 'unassigned'])))
    # Always include LAST_VERSION so that 'latest_version = vX_Y' compiles
    # even when no code point with that age is present in the dataset.
    last = float(LAST_VERSION)
    if last not in ages:
        ages = sorted(ages + [last])
    return ages
...
def write_numeric_data(characters, f):
    ...
    for s, characters in values.items():
        ...
        # Ensure non-8 arrays are never zero-length (zero-length C++ arrays are a GCC
        # extension that std::end() cannot bind by reference in GCC 14).
        if s != '8' and not characters:
            f.write("uni::detail::pair<char32_t, int{}_t>{{0x110000, 0}},".format(s))
        f.write("};") 
